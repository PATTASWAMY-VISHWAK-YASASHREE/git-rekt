...
export async function generateRoast(userData, repos, commits, events) {
  const roasts = [];
  let emojiCrimes = 0;
  ...
  if (isGeminiAvailable()) {
    try {
      const geminiResult = await generateGeminiRoasts(userData, repos, commits, events, analysisResults);
